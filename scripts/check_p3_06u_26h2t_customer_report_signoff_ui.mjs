...
const outputDir = path.resolve(process.env.P3_06U_26H2T_OUTPUT ?? path.join(rootDir, "output/p3_06u_26h2t_customer_report_signoff_ui"));
const backendPort = Number(process.env.P3_06U_26H2T_BACKEND_PORT ?? 8109);
const frontendPort = Number(process.env.P3_06U_26H2T_FRONTEND_PORT ?? 5209);
const chromePort = Number(process.env.P3_06U_26H2T_CHROME_PORT ?? 9249);
const backendUrl = `http://127.0.0.1:${backendPort}`;
const frontendUrl = `http://127.0.0.1:${frontendPort}/#quality`;
const chromeEndpoint = `http://127.0.0.1:${chromePort}`;
const dbPath = path.join(outputDir, "customer_report_signoff_smoke.sqlite");
const chromeProfile = path.join(outputDir, "chrome-profile");
const screenshotPath = path.join(outputDir, "customer-report-signoff.png");
...
async function seedData() {
  ...
  const owner = await apiJson("/api/auth/local-setup/owner", {
    method: "POST",
    body: JSON.stringify({
      tenant_slug: `h2t-signoff-${stamp}`,
      tenant_name: "H2T 客户签收记录空间",
      owner_name: "H2T 管理员",
      email: `h2t-owner-${stamp}@wanfa.local`,
      password: `H2TSmoke${stamp}!`
    })
  });
  ...
  const evaluationSet = await apiJson(`/api/tenants/${tenantId}/knowledge-evaluation-sets`, {
    ...
    body: JSON.stringify({
      ...
      cases: [
        {
          external_case_id: "h2t-final-answer-001",
          source_channel: "web",
          source_category: "售后",
          question: "标准产品保修期多久",
        ...
        {
          external_case_id: "h2t-final-answer-002",
          source_channel: "web",
...
async function runBrowserSmoke(seed) {
  ...
  try {
    ...
    if (!inspection.hasPanel || !inspection.hasExportButton || !inspection.hasSignoffForm || !inspection.hasSignoffStatus || !inspection.hasMaskedSigner) {
...
async function main() {
  ...
  console.log("PASS p3-06u-26h2t customer report signoff UI smoke");
