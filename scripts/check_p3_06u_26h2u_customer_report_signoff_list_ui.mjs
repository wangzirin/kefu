...
const outputDir = path.resolve(process.env.P3_06U_26H2U_OUTPUT ?? path.join(rootDir, "output/p3_06u_26h2u_customer_report_signoff_list_ui"));
const backendPort = Number(process.env.P3_06U_26H2U_BACKEND_PORT ?? 8110);
const frontendPort = Number(process.env.P3_06U_26H2U_FRONTEND_PORT ?? 5210);
const chromePort = Number(process.env.P3_06U_26H2U_CHROME_PORT ?? 9250);
const backendUrl = `http://127.0.0.1:${backendPort}`;
const frontendUrl = `http://127.0.0.1:${frontendPort}/#quality`;
const chromeEndpoint = `http://127.0.0.1:${chromePort}`;
const dbPath = path.join(outputDir, "customer_report_signoff_list_smoke.sqlite");
const chromeProfile = path.join(outputDir, "chrome-profile");
const screenshotPath = path.join(outputDir, "customer-report-signoff-list.png");
...
async function seedData() {
  ...
  const owner = await apiJson("/api/auth/local-setup/owner", {
    method: "POST",
    body: JSON.stringify({
      tenant_slug: `h2u-signoff-${stamp}`,
      tenant_name: "H2U 客户签收记录空间",
      owner_name: "H2U 管理员",
      email: `h2u-owner-${stamp}@wanfa.local`,
      password: `H2USmoke${stamp}!`
    })
  });
  ...
  const evaluationSet = await apiJson(`/api/tenants/${tenantId}/knowledge-evaluation-sets`, {
    ...
    body: JSON.stringify({
      name: "H2U 客户签收记录题集",
      description: "用于验证客户质量报告签收记录。",
      status: "active",
      evaluation_mode: "customer_service_retrieval",
      cases: [
        {
          external_case_id: "h2u-final-answer-001",
          source_channel: "web",
          source_category: "售后",
          question: "标准产品保修期多久",
        ...
        {
          external_case_id: "h2u-final-answer-002",
          source_channel: "web",
...
async function runBrowserSmoke(seed) {
  ...
  try {
    ...
    await waitForExpression(cdp, "Boolean(document.querySelector('[data-customer-report-signoff=\"p3-06u-26h2t\"]'))", 12000);
    await waitForExpression(cdp, "Boolean(document.querySelector('[data-customer-report-signoff-list=\"p3-06u-26h2u\"]'))", 12000);
    await evalValue(cdp, `(() => {
      const form = document.querySelector('[data-customer-report-signoff="p3-06u-26h2t"]');
      form.querySelector('[name="signer_name"]').value = '张三';
      form.querySelector('[name="signer_role"]').value = '运营负责人';
      form.querySelector('[name="signer_organization"]').value = '示例门店';
      form.querySelector('[name="notes"]').value = '客户确认本月报告可用于复盘，下月补充抖音样本。';
      form.requestSubmit();
      return true;
    })()`);
    await waitForExpression(cdp, "document.body.innerText.includes('已记录') && document.body.innerText.includes('客户确认')", 12000);
    const inspection = await evalValue(cdp, `(() => {
      ...
      return {
        ...
        hasSignoffForm: Boolean(document.querySelector('[data-customer-report-signoff="p3-06u-26h2t"]')),
        hasSignoffBoundary: Boolean(document.querySelector('[data-customer-report-signoff-boundary="p3-06u-26h2t"]')),
        hasSignoffList: Boolean(document.querySelector('[data-customer-report-signoff-list="p3-06u-26h2u"]')),
        hasSignoffListCount: text.includes('最近确认记录') && text.includes('2 条'),
        hasSignoffStatus: text.includes('已记录') && text.includes('客户确认'),
        hasMaskedSigner: text.includes('张*'),
        hasApiSignerRecord: text.includes('李*'),
        hasSignoffBoundaryText: text.includes('签收边界'),
        hasCustomerReadableTitle: text.includes('客户可读质量报告'),
        rawSignerLeaked: text.includes('张三'),
        noteLeaked: text.includes('客户确认本月报告可用于复盘'),
    ...
    if (!inspection.hasPanel || !inspection.hasExportButton || !inspection.hasSignoffForm || !inspection.hasSignoffList || !inspection.hasSignoffListCount || !inspection.hasSignoffStatus || !inspection.hasApiSignerRecord) {
      throw new Error(`Customer report signoff UI missing expected content: ${JSON.stringify(inspection)}`);
    }
    if (inspection.rawSignerLeaked || inspection.noteLeaked || inspection.rawQuestionLeaked || inspection.finalAnswerLeaked || inspection.externalWriteEnabled) {
      throw new Error(`Customer report signoff UI leaked unsafe state: ${JSON.stringify(inspection)}`);
    }
...
async function main() {
  ...
  const signoffNote = "客户负责人确认报告可用于本月复盘，后续补充抖音样本。";
  const signoff = await apiJson(`/api/tenants/${seed.tenantId}/customer-quality-report/signoffs`, {
    method: "POST",
  ...
  const signoffInspection = {
    schemaVersion: signoff.schema_version,
    statusLabel: signoff.signoff_status_label,
    signerDisplayName: signoff.signer_display_name,
    rawTextIncluded: signoff.raw_text_included,
    finalAnswerTextIncluded: signoff.final_answer_text_included,
    reviewerNotesIncluded: signoff.reviewer_notes_included,
    signerNameRawIncluded: signoff.signer_name_raw_included,
    electronicSignaturePerformed: signoff.electronic_signature_performed,
    formalContractSignoffPerformed: signoff.formal_contract_signoff_performed,
    modelCallPerformed: signoff.model_call_performed,
    externalPlatformWritePerformed: signoff.external_platform_write_performed,
    rawSignerLeaked: JSON.stringify(signoff).includes("李四"),
    noteLeaked: JSON.stringify(signoff).includes(signoffNote)
  };
  if (
    signoffInspection.schemaVersion !== "p3-06u-26h2t.customer_quality_report_signoff.v1" ||
    signoffInspection.statusLabel !== "确认通过，有备注" ||
    signoffInspection.signerDisplayName !== "李*" ||
    signoffInspection.rawTextIncluded ||
    signoffInspection.finalAnswerTextIncluded ||
    signoffInspection.reviewerNotesIncluded ||
    signoffInspection.signerNameRawIncluded ||
    signoffInspection.electronicSignaturePerformed ||
    signoffInspection.formalContractSignoffPerformed ||
    signoffInspection.modelCallPerformed ||
    signoffInspection.externalPlatformWritePerformed ||
    signoffInspection.rawSignerLeaked ||
    signoffInspection.noteLeaked
  ) {
    throw new Error(`Customer report signoff API boundary failed: ${JSON.stringify(signoffInspection)}`);
  }
  const signoffList = await apiJson(`/api/tenants/${seed.tenantId}/customer-quality-report/signoffs?page=1&page_size=8`, {
    headers: { Authorization: `Bearer ${seed.token}` }
  });
  const signoffListInspection = {
    schemaVersion: signoffList.schema_version,
    total: signoffList.total,
    firstSignerDisplayName: signoffList.items?.[0]?.signer_display_name,
    hasNotesHash: Boolean(signoffList.items?.[0]?.notes_hash),
    rawTextIncluded: signoffList.raw_text_included,
    finalAnswerTextIncluded: signoffList.final_answer_text_included,
    reviewerNotesIncluded: signoffList.reviewer_notes_included,
    signerNameRawIncluded: signoffList.signer_name_raw_included,
    externalPlatformWritePerformed: signoffList.external_platform_write_performed,
    rawSignerLeaked: JSON.stringify(signoffList).includes("李四"),
    noteLeaked: JSON.stringify(signoffList).includes(signoffNote)
  };
  if (
    signoffListInspection.schemaVersion !== "p3-06u-26h2u.customer_quality_report_signoff_list.v1" ||
    signoffListInspection.total !== 1 ||
    signoffListInspection.firstSignerDisplayName !== "李*" ||
    !signoffListInspection.hasNotesHash ||
    signoffListInspection.rawTextIncluded ||
    signoffListInspection.finalAnswerTextIncluded ||
    signoffListInspection.reviewerNotesIncluded ||
    signoffListInspection.signerNameRawIncluded ||
    signoffListInspection.externalPlatformWritePerformed ||
    signoffListInspection.rawSignerLeaked ||
    signoffListInspection.noteLeaked
  ) {
    throw new Error(`Customer report signoff list API boundary failed: ${JSON.stringify(signoffListInspection)}`);
  }
  ...
  fs.writeFileSync(summaryPath, JSON.stringify({
    ...
    exportInspection,
    signoffInspection,
    signoffListInspection,
    uiInspection
  }, null, 2));
  console.log("PASS p3-06u-26h2u customer report signoff list UI smoke");
