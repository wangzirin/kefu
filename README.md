...
- 后端：http://127.0.0.1:18080/health

如果本机已有 MaxKB 或其他服务占用 8080，不需要关闭它；标准运营版后端容器映射到宿主机 `18080`。
...
- 当前前端已能读取审核收件箱、批准进入待发送草稿、确认草稿、触发单条模拟发送、运行发送检查和生成官方渠道发送计划；outbox 支持发送前准备、确认、取消、防重复、dry-run 发送尝试、worker dry-run、连接器占位、发送计划、provider registry、官方 webhook 占位入口、失败状态、限流占位、回执占位、重试占位和审计。官方渠道真实发送器、真实外发、真实平台回执验签/解密和真实重试仍是下一阶段能力。
...
- P3-05E 仍不是企业微信真实外发：没有获取 access token，没有配置可信 IP 出站链路，没有调用发送消息 API，也没有打开 `OUTBOX_EXTERNAL_WRITE_ENABLED`。
...
- 当前仍不调用任何真实官方发送 API；`external_write=false`、`sent_at=null`、`external_message_id=""` 仍是硬边界。
